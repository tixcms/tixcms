...
class URL {
     ...
     * @return	string
     */
	public function site_url($uri = '', $show_suffix = true)
	{
		$CI =& get_instance();
	    if (is_array($uri))
		{
			$uri = implode('/', $uri) . $CI->config->item('url_suffix');
		}
		...
		if ($uri == '')
		{
			return $CI->config->item('base_url') . ( $CI->config->item('index_page') 
                ?  $CI->config->item('index_page') . $CI->config->item('url_suffix') : '');
		}
		else
		{
    ...
    public function site_url_protected($uri = '', $show_suffix = true)
    {
        $extra = (strpos($uri, '?') === false ? '?' : '&') .'csrf_token='. \Security::csrf_generate_hash(); 
        
        return $this->site_url($uri, $show_suffix) . $extra;
    }
    ...
    function anchor_protected($uri = '', $title = '', $attributes = '', $extra = '')
    {
        $extra .= (strpos($extra, '?') === false ? '?' : '&') .'csrf_token='. \Security::csrf_generate_hash(); 
        
        return $this->anchor($uri, $title, $attributes, $extra);
    }
     ...
     * @return	string
     */
	public function auto_link($str, $type = 'both', $popup = FALSE)
	{
		if ($type != 'email')
		{
     ...
     * Library's set_header() public function.
     *
     * @access	public
     * @param	string	the URL
     ...
     * @return	string
     */
	public function redirect($uri = '', $extra = '', $method = 'location', $http_response_code = 302)
	{
     ...
     *
     * Some of the public functions use this
     *
     * @access	private
     * @param	array
     * @param	bool
